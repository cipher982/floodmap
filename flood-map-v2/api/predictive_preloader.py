...
class PredictiveTilePreloader:
    ...
    def __init__(self, max_preload_workers: int = None):
        ...
        self.preload_pool = ThreadPoolExecutor(
            ...
            thread_name_prefix="tile-preload"
        )
        ...
        logger.info(f"🧠 Predictive preloader: {self.max_workers} cores for tile anticipation")
    
    def record_tile_request(self, z: int, x: int, y: int, water_level: float, 
                           user_session: str = "default"):
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                if self._is_valid_tile(request.z, new_x, new_y):
                    predictions.append(TileRequest(
                        z=request.z,
                        x=new_x,
                        y=new_y,
...
import os
predictive_preloader = PredictiveTilePreloader(max_preload_workers=min(12, os.cpu_count()))