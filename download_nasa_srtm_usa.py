        ...
        for attempt in range(RETRY_COUNT):
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.warning(
                        f"Download attempt {attempt + 1} failed for {tile['name']}: {result.stderr}"
                    )
                ...
                logger.warning(
                    f"Download timeout attempt {attempt + 1} for {tile['name']}"
                )
                if attempt == RETRY_COUNT - 1:
                    return {
                        "success": False,
...
def main():
    ...
    print(
        f"📊 Results: {success_count} downloaded, {exists_count} existing, {error_count} errors"
    )
