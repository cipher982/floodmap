...
def test_vector_tiles() -> Dict:
    ...
    for z, x, y in test_tiles:
        try:
            ...
            # Test new v1 API
            v1_api_url = f"http://localhost:8000/api/v1/tiles/vector/usa/{z}/{x}/{y}.pbf"
            ...
            success = (
                ...
                legacy_response.status_code == 200 and
                v1_response.status_code == 200 and
            ...
            print(f"  {status} Vector tile {z}/{x}/{y}: Direct={direct_response.status_code}({direct_size}b), Legacy={legacy_response.status_code}({legacy_size}b), V1={v1_response.status_code}({v1_size}b)")
...
def test_elevation_tiles() -> Dict:
    ...
    for water_level, z, x, y in test_tiles:
        try:
            ...
            v1_elevation_url = f"http://localhost:8000/api/v1/tiles/elevation/{z}/{x}/{y}.png"
            ...
            success = (
                legacy_response.status_code in [200, 204] and 
                v1_flood_response.status_code in [200, 204] and
                v1_elevation_response.status_code in [200, 204] and
                (legacy_size > 100 or legacy_response.status_code == 204) and
                (v1_flood_size > 100 or v1_flood_response.status_code == 204) and
            ...
            print(f"  {status} Elevation {water_level}m/{z}/{x}/{y}: Legacy={legacy_response.status_code}({legacy_size}b), V1Flood={v1_flood_response.status_code}({v1_flood_size}b), V1Elev={v1_elevation_response.status_code}({v1_elevation_size}b) {duration_ms:.0f}ms")
