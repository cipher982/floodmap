...
class ElevationProcessor:
    ...
    def ingest_compressed(self, region_id: str) -> Optional[Path]:
        ...
        try:
            ...
            with rasterio.open(
                ...
                height=metadata['shape'][0], width=metadata['shape'][1],
                count=1, dtype=metadata['dtype'], crs=metadata['crs'],
                transform=transform, nodata=metadata['nodata_value']
            ) as dst:
    ...
    def create_elevation_tile(self, src, xtile, ytile, zoom, tile_size=256):
        ...
        try:
            ...
            bounds = transform_bounds('EPSG:4326', src.crs, lon_west, lat_south, lon_east, lat_north)
        except:
        ...
        if (bounds[2] <= src_bounds.left or bounds[0] >= src_bounds.right or
            bounds[3] <= src_bounds.bottom or bounds[1] >= src_bounds.top):
        ...
        try:
            ...
            data = src.read(1, window=window, out_shape=(tile_size, tile_size), resampling=Resampling.cubic)
                ...
                try:
                    with rasterio.open(tif_file) as src:
                        ...
                        for zoom in range(8, 13):
                            ...
                            min_x, max_y = self.deg2num(bounds[1], bounds[0], zoom)  # bottom-left -> top-left
                            max_x, min_y = self.deg2num(bounds[3], bounds[2], zoom)  # top-right -> bottom-right
                            ...
                            total_tiles_for_zoom = (max_x - min_x + 1) * (max_y - min_y + 1)
                            
                            with tqdm(total=total_tiles_for_zoom, desc=f"Zoom {zoom}", leave=False) as pbar:
                                for x in range(min_x, max_x + 1):
                                    for y in range(min_y, max_y + 1):
                                        # Create tile
                                        tile_img = self.create_elevation_tile(src, x, y, zoom)
                                        if tile_img:
                                            # Save tile
    ...
    def deploy_region(self, regions: List[str], max_workers: int = 4) -> Dict[str, bool]:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_region):
                ...
                if completed % 10 == 0 or current_time - last_progress_time > 5:
                    ...
                    progress_line = (f"✅ {completed:4d}/{len(regions)} ({percent_complete:5.1f}%) | "
        ...
        if tiles_dir.exists():
            ...
            tile_size_mb = sum(f.stat().st_size for f in tiles_dir.rglob("*.png")) / (1024 * 1024)
        else:
            tile_count = 0
            tile_size_mb = 0
        ...
        if tile_count > 0:
            ...
            print(f"   ⚠️  No PNG tiles generated")
...
def get_regions_by_area() -> Dict[str, List[str]]:
    ...
    return {
        ...
        'gulf_coast': [f'n{lat}_w{lng:03d}_1arc_v3' for lat in range(25, 31) for lng in range(82, 99)],
    }

def main():
    """Main processing pipeline."""
    parser = argparse.ArgumentParser(description="Process elevation data for flood mapping")
    ...
    parser.add_argument('--tiles-only', action='store_true', help='Skip ingestion, just generate tiles from existing TIFs')
    ...
    processor = ElevationProcessor(
        data_dir=Path.cwd(),
        output_dir=Path("processed_data/tiles")
    )
    ...
    if args.test:
        ...
        # Default test
        tiles = [all_tiles[0]] if all_tiles else []
        logger.info(f"No option specified, testing with: {tiles[0] if tiles else 'no tiles found'}")
