...
class RegionalMapProcessor:
    ...
    def download_region(self, region_key: str) -> bool:
        ...
        logger.info(f"🌍 Downloading {region['name']} ({region['expected_size_mb']}MB expected)...")
        ...
        try:
            def progress_hook(block_num, block_size, total_size):
                if total_size > 0:
                    ...
                    print(f"\r📥 {region['name']}: {percent:.1f}% ({mb_downloaded:.1f}/{total_mb:.1f}MB)", end='', flush=True)
    ...
    def process_region(self, region_key: str) -> bool:
        ...
        cmd = [
            "java", "-Xmx8g",  # 8GB heap size
            "-jar", self.planetiler_jar,
            "--download",  # Download required data sources
            "--osm-file", str(pbf_path),
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                size_mb = mbtiles_path.stat().st_size / (1024 * 1024)
                logger.info(f"✅ Processed {region['name']} in {processing_time/60:.1f}min → {size_mb:.1f}MB")
                return True
            else:
        ...
        for region_key, region in sorted_regions[:max_regions]:
            ...
            if self.download_region(region_key):
                ...
                if self.process_region(region_key):
                    ...
                    logger.warning(f"⚠️ Processing failed for {region['name']}, continuing...")
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download and process regional map data")
    parser.add_argument("--region", help="Process specific region", choices=list(REGIONS.keys()))
    parser.add_argument("--max-regions", type=int, default=3, help="Max regions to process")
