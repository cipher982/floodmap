...
def process_tile(z: int, x: int, y: int, out_dir: Path, generate_br: bool, generate_gz: bool, write_raw: bool, skip_existing: bool = True) -> Optional[dict]:
    ...
    try:
        arr = elevation_loader.get_elevation_for_tile(x, y, z, tile_size=TILE_SIZE)
    except Exception:
        arr = None

    # Fallback: if no data extracted (pure ocean/outside coverage), write an all-NODATA tile.
    if arr is None:
        payload = np.full((TILE_SIZE, TILE_SIZE), 65535, dtype=np.uint16).tobytes()
    else:
...
def generate_for_zoom(z: int, bbox: Tuple[float, float, float, float], out_dir: Path, workers: int, generate_br: bool, generate_gz: bool, write_raw: bool, skip_existing: bool = True, max_tasks_inflight: int = 2000) -> dict:
    ...
    with cf.ProcessPoolExecutor(max_workers=workers) as pool:
        ...
        def submit_next(batch: Iterable[Tile]) -> None:
            nonlocal submitted
            for t in batch:
                fut = pool.submit(process_tile, t.z, t.x, t.y, out_dir, generate_br, generate_gz, write_raw, skip_existing)
                futures.add(fut)
                submitted += 1
...
def main() -> None:
    ...
    if args.bbox:
        ...
        bbox = discover_coverage_bbox_from_metadata(ELEVATION_DATA_DIR)
    ...
    overall = {
        ...
        'source_dir': str(ELEVATION_DATA_DIR),
    ...
    for z in range(zoom_min, zoom_max + 1):
        ...
        print(f"  ✓ z={z} wrote {zsum['tiles_written']} tiles in {zsum['elapsed_sec']}s")

    overall['totals']['elapsed_sec'] = round(time.time() - t_start, 2)
    write_manifest(out_dir, overall)
    print(f"✅ Done. Wrote {overall['totals']['tiles_written']} tiles across z={zoom_min}..{zoom_max}.")
