...
class PrecompressionGenerator:
    ...
    def generate_tile_data(self, z: int, x: int, y: int) -> bytes | None:
        ...
        try:
            ...
            elevation_data = self.elevation_loader.get_elevation_for_tile(
                x, y, z, tile_size=self.tile_size
            )

            if elevation_data is None:
                # Return empty elevation data
                empty_data = np.full(
                    (self.tile_size, self.tile_size), 65535, dtype=np.uint16
                )
    ...
    def generate_sample_tiles(
        self, zoom_levels: list[int] = [10, 12, 14], tiles_per_zoom: int = 5
            ...
            for lat, lon in sample_locations:
                ...
                if overlapping_files:
                    ...
                    if tile_data and len(tile_data) > 0:
                        ...
                        tiles_generated += 1
                        logger.info(
                            f"Generated tile {zoom}/{x}/{y} ({len(tile_data)} bytes)"
                        )
    ...
    def save_tile_variants(self, z: int, x: int, y: int, tile_data: bytes):
        ...
        if brotli:
            ...
            logger.debug(
                f"Brotli: {len(tile_data)} → {len(compressed_br)} bytes ({len(compressed_br) / len(tile_data) * 100:.1f}%)"
            )
        ...
        logger.debug(
            f"Gzip: {len(tile_data)} → {len(compressed_gz)} bytes ({len(compressed_gz) / len(tile_data) * 100:.1f}%)"
        )
    ...
    def generate_manifest(self, generated_tiles: set[tuple[int, int, int]]):
        ...
        logger.info(
            f"Generated manifest with {len(generated_tiles)} tiles: {manifest_path}"
        )


def main():
    parser = argparse.ArgumentParser(
        description="Generate pre-compressed tiles for performance testing"
    )
    parser.add_argument(
        "--source-dir",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("tools/performance_testing/precompressed_tiles"),
    ...
    generated_tiles = generator.generate_sample_tiles(
        args.zoom_levels, args.tiles_per_zoom
    )
    generator.generate_manifest(generated_tiles)

    logger.info(
        f"Pre-compression complete! Generated {len(generated_tiles)} tiles in {args.output_dir}"
    )
