...
def generate_tiles_for_file(zst_path, output_dir, zoom_levels):
    ...
    # Get file bounds
    file_lat_top, file_lat_bottom, file_lon_left, file_lon_right = get_file_bounds(metadata)
    ...
    for zoom in zoom_levels:
        ...
        for x in range(min_x, max_x + 1):
            for y in range(min_y, max_y + 1):
                # Get tile bounds
                tile_lat_top, tile_lat_bottom, tile_lon_left, tile_lon_right = num2deg(x, y, zoom)
                
                # Check if tile overlaps with this elevation file
                if (tile_lat_bottom < file_lat_top and tile_lat_top > file_lat_bottom and
                    tile_lon_left < file_lon_right and tile_lon_right > file_lon_left):
                    ...
                    sample_data = elevation_array[::elevation_array.shape[0]//TILE_SIZE or 1, 
...
def main():
    ...
    logger.info(f"Complete! Generated {total_tiles} tiles from {len(zst_files)} files in {elapsed:.1f}s")
