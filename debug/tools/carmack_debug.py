...
class TileReality:
    ...
    def capture_browser_tile(self, z, x, y, water_level):
        ...
        try:
            ...
            elevation[~nodata_mask] = (
                elevation[~nodata_mask] / 65534.0
            ) * 9500.0 - 500.0
            ...
            rgba = color_mapper.elevation_array_to_rgba(
                elevation, water_level, no_data_value=-32768
            )
            ...
            analysis = {
                ...
                "shape": pixels.shape,
                "unique_colors": len(
                    np.unique(pixels.reshape(-1, pixels.shape[2]), axis=0)
                ),
                "mean_color": np.mean(pixels, axis=(0, 1)).astype(int).tolist(),
                "is_solid": len(np.unique(pixels.reshape(-1, pixels.shape[2]), axis=0))
                == 1,
    ...
    def test_water_level_consistency(self, z, x, y, water_levels):
        ...
        for wl, analysis in results:
            if "error" not in analysis:
                ...
                print(
                    f"   {wl}m: {analysis['unique_colors']} colors, {solid_str}, hash={analysis['hash']}"
                )
            else:
        ...
        if unique_hashes == 1:
            print("   🚨 PROBLEM: All water levels produce identical tiles!")
        elif unique_hashes == len(results):
            print("   ✅ Good: Each water level produces different tiles")
        else:
            print("   ⚠️ Mixed: Some water levels produce identical tiles")
    ...
    def test_coordinate_consistency(self, water_level):
        ...
        for coord, analysis in results:
            if "error" not in analysis:
                solid_str = "SOLID" if analysis["is_solid"] else "VARIED"
                print(
                    f"   {coord}: {analysis['unique_colors']} colors, {solid_str}, hash={analysis['hash']}"
                )
            else:
    ...
    def test_cache_behavior(self, z, x, y, water_level, iterations=3):
        ...
        print(
            f"🔄 Testing cache consistency for {z}/{x}/{y} at {water_level}m ({iterations} requests)"
        )
        ...
        print("\n🎯 Cache Analysis:")
        if unique_hashes == 1:
            print("   ✅ Consistent: All requests return identical tiles")
        else:
            print(
                f"   🚨 INCONSISTENT: {unique_hashes} different tiles from {iterations} requests!"
            )
