...
from api.elevation_loader import ElevationDataLoader  
...
def trace_tile_generation(z, x, y, water_level):
    ...
    print(f"   Bounds: lat {lat_bottom:.6f} to {lat_top:.6f}, lon {lon_left:.6f} to {lon_right:.6f}")
        ...
        try:
            ...
            if name.startswith('n') and '_w' in name:
                ...
                if lat_overlap and lon_overlap:
                    ...
                    print(f"      MATCH: {name} covers lat {file_lat}-{file_lat_max}, lon {file_lon}-{file_lon_max}")
    ...
    if len(unique_colors) == 1:
        ...
        print(f"   🚨 SOLID COLOR: RGBA({color[0]}, {color[1]}, {color[2]}, {color[3]})")
        
        # Check what this color represents
        if np.array_equal(color, color_mapper.FLOODED_COLOR):
            print(f"   This is FLOODED_COLOR - entire tile is below water level!")
        elif np.array_equal(color, color_mapper.SAFE_COLOR):
            print(f"   This is SAFE_COLOR - entire tile is above safe threshold!")
    else:
        print(f"   ✅ Color variation detected")
        for i, color in enumerate(unique_colors[:5]):
            print(f"      Color {i+1}: RGBA({color[0]}, {color[1]}, {color[2]}, {color[3]})")
    ...
    return {
        ...
        'elevation_range': (np.min(elevation_data), np.max(elevation_data)) if elevation_data is not None else None,
