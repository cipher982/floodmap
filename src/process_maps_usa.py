...
class USAMapProcessor:
    ...
    def check_prerequisites(self):
        ...
        try:
            result = subprocess.run(
                [java_cmd, "-version"], capture_output=True, text=True, check=True
            )
        ...
        if free_space_gb < 50:  # Need at least 50GB for processing
            logger.error(
                f"❌ Insufficient disk space: {free_space_gb:.1f}GB (need 50GB+)"
            )
            return False
    ...
    def process_usa_mbtiles(self, maxzoom=12, memory_gb=8):
        ...
        logger.info(
            f"Input: {self.usa_osm_file} ({self.usa_osm_file.stat().st_size / (1024**3):.1f}GB)"
        )
        ...
        cmd = [
            ...
            "-jar",
            str(self.planetiler_jar),
            "--osm-path",
            str(self.usa_osm_file),
            "--output",
            str(self.usa_mbtiles),
            "--download-dir",
            str(self.auxiliary_data),  # Use existing auxiliary data
            "--force",  # Overwrite existing output file
            "--minzoom",
            "0",
            "--maxzoom",
            str(maxzoom),  # Use parameter
        ]
        ...
        try:
            ...
            if process.returncode != 0:
                logger.error(
                    f"❌ Planetiler failed with return code {process.returncode}"
                )
                return False
        ...
        if self.usa_mbtiles.exists():
            ...
            logger.info(
                f"✅ USA processing completed in {processing_time / 60:.1f} minutes"
            )
    ...
    def run_complete_processing(self, maxzoom=12, memory_gb=8):
        ...
        logger.info(
            "🗺️ USA-wide map tiles are now available for the flood mapping system"
        )
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process USA map tiles (roads, borders, cities)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--maxzoom", type=int, default=12, help="Maximum zoom level (default: 12)"
    )
    parser.add_argument(
        "--memory", type=int, default=8, help="Memory allocation in GB (default: 8)"
    )
    ...
    if args.dry_run:
        ...
        logger.info(
            f"📁 Input: {processor.usa_osm_file} ({processor.usa_osm_file.stat().st_size / (1024**3):.1f}GB)"
        )
    ...
    success = processor.run_complete_processing(
        maxzoom=args.maxzoom, memory_gb=args.memory
    )
