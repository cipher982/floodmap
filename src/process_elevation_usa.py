...
def compress_tile(
    input_path: Path, output_dir: Path, compression_level: int = 3
) -> dict:
...
def test_decompression(compressed_file: Path, metadata_file: Path) -> bool:
    ...
    if elevation_data.shape != tuple(metadata["shape"]):
        logging.error(
            f"Shape mismatch: expected {metadata['shape']}, got {elevation_data.shape}"
        )
        return False
...
def estimate_processing_time(tif_files: list, sample_size: int = 3) -> float:
    ...
    logging.info(
        f"Estimated total processing time: {estimated_total / 60:.1f} minutes ({estimated_total / 3600:.1f} hours)"
    )
...
def compress_directory(
    input_dir: Path,
    output_dir: Path,
    test_decompression_flag: bool = True,
    ...
    if sample_count and sample_count < len(tif_files):
        ...
        logging.info(
            f"🧪 SAMPLE MODE: Processing {len(tif_files)} random files out of {len(list(input_dir.glob('*.tif')))} total"
        )
    else:
                    ...
                    try:
                        ...
                        if test_decompression_flag:
                            ...
                            if not test_decompression(compressed_file, metadata_file):
                                logging.error(
                                    f"Decompression test failed for {result['tile_id']}"
                                )
...
def _run_prechecks(
    ...
    if "bbox_audit" in reports:
        ...
        logging.info(
            f"Expected {r.get('expected_count')} tiles; Present {r.get('present_count')} tiles"
        )
        ...
        if missing:
            ...
            if fail_on_missing:
                raise SystemExit(
                    "Missing expected SRTM tiles in coverage area — aborting."
                )
    if "interior_holes" in reports:
        r = reports["interior_holes"]
        if r.get("suspicious_count", 0) > 0:
            logging.warning(
                "Suspicious interior holes detected: %d (first 20: %s)",
                r["suspicious_count"],
                ", ".join(r["suspicious"][:20]),
            )
...
def main():
    ...
    parser.add_argument(
        "--input", required=True, help="Input directory containing .tif files"
    )
    parser.add_argument(
        "--output", required=True, help="Output directory for compressed files"
    )
    parser.add_argument(
        "--compression-level", type=int, default=3, help="ZSTD compression level (1-22)"
    )
    parser.add_argument(
        "--no-test", action="store_true", help="Skip decompression testing"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose logging")
    parser.add_argument("--workers", type=int, help="Number of parallel workers")
    parser.add_argument(
        "--samples", type=int, help="Process only N random sample files (for testing)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--precheck", action="store_true", help="Audit coverage before compressing"
    )
    parser.add_argument(
        "--region",
        help="Region preset for expected coverage (e.g., florida, usa-conus)",
    )
    parser.add_argument(
        "--bbox",
        nargs=4,
    ...
    parser.add_argument(
        "--check-holes",
        action="store_true",
        help="Detect interior holes based on present tiles",
    )
    ...
    if args.dry_run:
        ...
        logging.info("🔍 DRY RUN: Would process USA elevation data")
        logging.info(f"📁 Input: {input_dir} ({len(tif_files)} SRTM tiles)")
        logging.info(f"📁 Output: {output_dir}")
        logging.info(
            f"🎯 Samples: {min(sample_count, len(tif_files))}/{len(tif_files)} files"
        )
