...
_CITY_PAGES: Final[tuple[CityPage, ...]] = (
    ...
    CityPage(
        ...
        city_name="Galveston",
        state_name="Texas",
        focus_areas="Galveston Bay, the seawall corridor, and low ground across the barrier island",
        local_context="Galveston sits on a narrow barrier island with Gulf exposure on one side and bay on the other — one of the most storm-surge-vulnerable cities in the U.S.",
        default_view_state=FloodmapViewState(lat=29.3013, lng=-94.7977, zoom=11.2),
        related_city_keys=(("tx", "houston"), ("la", "new-orleans"), ("fl", "tampa")),
    ),
    CityPage(
        state_slug="fl",
        ...
        state_name="Florida",
        focus_areas="the New River, Intracoastal Waterway, and low-lying canal neighborhoods",
        local_context="Fort Lauderdale is threaded with canals and sits on flat coastal ground, making tidal and storm surge flooding especially relevant across broad swaths of the city.",
        default_view_state=FloodmapViewState(lat=26.1224, lng=-80.1373, zoom=10.6),
        related_city_keys=(("fl", "miami"), ("fl", "tampa"), ("sc", "charleston")),
    ),
    CityPage(
        state_slug="nc",
        city_slug="wilmington",
        city_name="Wilmington",
        state_name="North Carolina",
        focus_areas="the Cape Fear River, Wrightsville Beach shoreline, and low ground across the coastal plain",
        local_context="Wilmington sits at the mouth of the Cape Fear River on a low coastal plain, and is a frequent landfall target for Atlantic hurricanes.",
        default_view_state=FloodmapViewState(lat=34.2257, lng=-77.9447, zoom=10.4),
        related_city_keys=(("sc", "charleston"), ("va", "norfolk"), ("ga", "savannah")),
    ),
    CityPage(
        state_slug="ms",
        city_slug="biloxi",
        ...
        focus_areas="Biloxi Bay, the Gulf shoreline, and low barrier peninsula neighborhoods",
        local_context="Biloxi's casino district and residential peninsula have almost no elevation buffer from Gulf storm surge — a narrow barrier between open water and the bay.",
        default_view_state=FloodmapViewState(lat=30.3960, lng=-88.8853, zoom=10.8),
        related_city_keys=(
            ("la", "new-orleans"),
            ("al", "birmingham"),
            ("fl", "tampa"),
    ...
    CityPage(
        ...
        state_name="Florida",
        focus_areas="the St. Johns River, Duval County creek corridors, and low neighborhoods downstream from the river bend",
        local_context="Jacksonville's size and the meandering St. Johns River mean flood risk varies significantly by neighborhood — river flooding combines with storm surge risk near the coast.",
        default_view_state=FloodmapViewState(lat=30.3322, lng=-81.6557, zoom=10.0),
        related_city_keys=(("fl", "tampa"), ("fl", "miami"), ("ga", "savannah")),
    ),
)
