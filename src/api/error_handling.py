...
def create_fallback_tile(
    tile_size: int = 256, color: tuple = (128, 128, 128, 64)
) -> bytes:
    ...
    try:
        ...
        center = tile_size // 2
        draw.ellipse(
            [center - 2, center - 2, center + 2, center + 2], fill=(100, 100, 100, 128)
        )
...
def safe_tile_generation(func: Callable) -> Callable:
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            return Response(
                content=create_fallback_tile(
                    color=(180, 100, 100, 96)
                ),  # Slightly red to indicate error
                media_type="image/png",
                headers={
...
class HealthMonitor:
    ...
    def get_stats(self) -> dict:
        ...
        cache_hit_rate = self.elevation_cache_hits / max(
            self.elevation_cache_hits + self.elevation_cache_misses, 1
        )
