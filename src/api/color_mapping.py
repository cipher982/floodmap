...
class FloodRiskColorMapper:
    ...
    def elevation_to_risk_level(self, elevation: float, water_level: float) -> float:
        ...
        if relative_elevation >= self.SAFE_THRESHOLD:
            ...
            return (
                (self.SAFE_THRESHOLD - relative_elevation)
                / (self.SAFE_THRESHOLD - self.CAUTION_THRESHOLD)
                * 0.33
            )
            ...
            return (
                0.33
                + (self.CAUTION_THRESHOLD - relative_elevation)
                / (self.CAUTION_THRESHOLD - self.DANGER_THRESHOLD)
                * 0.33
            )
            ...
            return (
                0.66
                + (self.DANGER_THRESHOLD - relative_elevation)
    ...
    def _blend_colors(
        ...
        return tuple(int(color1[i] * (1 - t) + color2[i] * t) for i in range(4))

    def elevation_array_to_rgba(
        self,
        ...
        danger_mask = (elevation_data >= (water_level + self.DANGER_THRESHOLD)) & (
            elevation_data < (water_level + self.CAUTION_THRESHOLD)
        )
        caution_mask = (elevation_data >= (water_level + self.CAUTION_THRESHOLD)) & (
            elevation_data < (water_level + self.SAFE_THRESHOLD)
        )
    ...
    def elevation_array_to_topographical_rgba(
        self, elevation_data: np.ndarray, no_data_value: float | None = None
    ) -> np.ndarray:
        """
        ...
        for i in range(len(self.ELEVATION_THRESHOLDS) - 1):
            ...
            band_mask = (
                (elevation_data >= min_elev) & (elevation_data < max_elev) & valid_mask
            )
