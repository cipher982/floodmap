...
class RateLimiter:
    ...
    def _is_redis_available(self) -> bool:
        ...
        if (self.redis_failures >= self.circuit_breaker_threshold and 
            now - self.last_redis_failure < self.circuit_breaker_timeout):
            return False
    ...
    async def check_rate_limit(self, request: Request, endpoint: str = "api", limit: int = 60) -> None:
        """
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Apply rate limiting
        try:
            endpoint = request.url.path.split('/')[1] if len(request.url.path.split('/')) > 1 else "api"
