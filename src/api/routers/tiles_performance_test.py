...
PERFORMANCE_TEST_DATA_DIR = (
    PROJECT_ROOT / "tools" / "performance_testing" / "precompressed_tiles"
)
...
async def get_runtime_compressed_tile(
    ...
    try:
        ...
        compression_ratio = (
            len(payload) / len(elevation_data) if len(elevation_data) > 0 else 1.0
        )
        record_performance_metric(
            "runtime", latency_ms, len(payload), compression_ratio
        )
    ...
    try:
        ...
        if encoding == "br" and brotli:
            ...
            if compressed_path.exists():
                ...
                record_performance_metric(
                    "precompressed", latency_ms, compressed_size, compression_ratio
                )
...
async def get_memory_cached_tile(
    ...
    try:
        ...
        if cached_data is not None:
            ...
            record_performance_metric(
                "memory_cache", latency_ms, len(cached_data), compression_ratio
            )
        ...
        latency_ms = (time.time() - start_time) * 1000
        record_performance_metric(
            "memory_cache", latency_ms, len(payload), compression_ratio
        )
