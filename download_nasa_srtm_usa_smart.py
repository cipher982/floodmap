        ...
        for attempt in range(RETRY_COUNT):
            try:
                ...
                if result.returncode == 0:
                    ...
                    return {'success': False, 'tile': tile['name'], 'error': '404 - Tile not available on NASA server'}
                else:
                    if attempt == RETRY_COUNT - 1:
                        return {'success': False, 'tile': tile['name'], 'error': f"Download failed: HTTP {result.returncode}"}
...
def main():
    ...
    if not_available_count > 0:
        print(f"ℹ️  {not_available_count} tiles not available on NASA (likely pure ocean areas)")
