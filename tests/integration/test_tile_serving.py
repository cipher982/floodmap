...
class TestTileServing:
    ...
    def test_elevation_tile_serving_v1(self, api_client):
        ...
        assert response.status_code == 200, (
            f"V1 elevation tile failed: {response.status_code}"
        )
    ...
    def test_vector_tile_serving_v1(self, api_client):
        ...
        assert response.status_code == 200, (
            f"V1 vector tile failed: {response.status_code}"
        )
    ...
    def test_flood_tile_serving_v1(self, api_client):
        ...
        assert response.status_code in [200, 204], (
            f"V1 flood tile failed: {response.status_code}"
        )
    ...
    def test_legacy_vs_v1_compatibility(self, api_client):
        ...
        legacy_response = api_client.get(
            f"/api/tiles/elevation/{water_level}/{z}/{x}/{y}.png"
        )
        v1_response = api_client.get(
            f"/api/v1/tiles/flood/{water_level}/{z}/{x}/{y}.png"
        )
...
class TestTileURLDebug:
    ...
    def test_maplibre_tile_urls(self, api_client):
        ...
        for pattern in expected_patterns:
            ...
            pattern_found = any(
                pattern.replace("{z}", "{").replace("{x}", "{").replace("{y}", "{")
                in line
                for line in tile_lines
            )
        ...
        for url in failing_urls:
            ...
            if response.status_code >= 400:
                ...
                if response.status_code == 404:
                    print(
                        "   💡 This suggests the tile doesn't exist or route isn't working"
                    )
