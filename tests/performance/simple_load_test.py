...
def concurrent_load_test(
    base_url: str, concurrent_users: int = 10, duration_seconds: int = 30
):
    """Run concurrent load test similar to wrk."""
    print(
        f"🚀 Running load test: {concurrent_users} concurrent users for {duration_seconds}s"
    )
    ...
    if successful_requests:
        ...
        print(
            f"Successful:         {len(successful_requests)} ({len(successful_requests) / len(results) * 100:.1f}%)"
        )
        ...
        if avg_duration < 100:
            ...
            print("🟡 Performance: ACCEPTABLE (100-300ms)")
        else:
            print("🔴 Performance: NEEDS OPTIMIZATION (>300ms)")
