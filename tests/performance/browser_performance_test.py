...
class BrowserPerformanceTest:
    ...
    async def setup_browser(self):
        ...
        self.page.on(
            "request",
            lambda request: self.network_requests.append(
    ...
    async def test_zoom_performance(
        self, zoom_levels: list[int] = None
    ) -> dict[str, Any]:
    ...
    async def test_water_level_change(
        ...
        return {
            ...
            "avg_change_time": statistics.mean(
                [r["duration_ms"] for r in level_results]
            ),
    ...
    async def test_heavy_interaction(self) -> dict[str, Any]:
        ...
        for i in range(5):
            ...
            await self.page.evaluate(
                f"window.map && window.map.setZoom && window.map.setZoom({11 + i % 2})"
            )
            await asyncio.sleep(0.5)

            # Quick pan
            await self.page.evaluate(
                f"window.map && window.map.panBy && window.map.panBy([{i * 20}, {i * 10}])"
            )
            await asyncio.sleep(0.5)
        ...
        return {
            ...
            "avg_tiles_per_second": total_tiles / (end_time - start_time)
        ...
        try:
            ...
            if tile_requests:
                ...
                results["tile_analysis"] = {
                    ...
                    "success_rate": len(
                        [r for r in tile_requests if r["status"] == 200]
                    )
                    / len(tile_requests),
                }
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Browser performance test for flood map"
    )
    parser.add_argument(
        "--url", default="http://localhost:5002", help="Base URL for the application"
    )
