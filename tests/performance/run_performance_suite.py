...
class PerformanceTestSuite:
    ...
    def run_synthetic_benchmarks(self) -> dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                with open("/tmp/synthetic_results.json") as f:
                    return json.load(f)
            else:
    ...
    def analyze_combined_results(
        self, synthetic: dict[str, Any], browser: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if analysis["synthetic_available"] and analysis["browser_available"]:
            ...
            if "single_tile" in synthetic and "tile_analysis" in browser:
                ...
                analysis["comparison"]["tile_performance"] = {
                    ...
                    "overhead_percentage": ((browser_avg - synth_avg) / synth_avg * 100)
                    if synth_avg > 0
                    else 0,
                }
            ...
            if analysis["synthetic_available"]:
                if "single_tile" in synthetic:
                    ...
                    if avg_tile_time > 500:
                        recommendations.append(
                            "🔴 CRITICAL: Tile generation >500ms - optimize elevation loading"
                        )
                    elif avg_tile_time > 200:
                        recommendations.append(
                            "🟡 WARNING: Tile generation >200ms - consider caching improvements"
                        )
                    else:
                ...
                if "concurrent_tiles" in synthetic:
                    ...
                    if concurrent_perf > avg_tile_time * 2:
                        recommendations.append(
                            "🔴 CRITICAL: Poor concurrent performance - threading issues"
                        )
            ...
            if analysis["browser_available"]:
                if "tile_analysis" in browser:
                    ...
                    if success_rate < 0.95:
                        recommendations.append(
                            f"🔴 CRITICAL: {success_rate:.1%} tile success rate - network/server issues"
                        )
                    elif success_rate < 0.99:
                        recommendations.append(
                            f"🟡 WARNING: {success_rate:.1%} tile success rate"
                        )
    ...
    def print_comprehensive_report(
        ...
        if "tile_performance" in analysis["comparison"]:
            ...
            print(
                f"  Browser Overhead: +{comp['browser_overhead_ms']:.1f} ms ({comp['overhead_percentage']:.1f}%)"
            )
        ...
        print("  • Local (M3 Max): Optimize for CPU/memory efficiency")
        print("  • VPS Deployment: Focus on I/O optimization and caching")
        print("  • Network: Add CDN for static tiles, consider HTTP/2")
        print("  • Scaling: Implement Redis cache, horizontal scaling")
...
async def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:5002", help="Base URL for testing"
    )
