...
def profile_with_load(pid: int, base_url: str, duration: int = 30):
    ...
    if load_requests:
        ...
        if successful:
            ...
            print(
                f"Successful:        {len(successful)} ({len(successful) / len(load_requests) * 100:.1f}%)"
            )
            print(f"Average Duration:  {statistics.mean(durations):.1f} ms")
            print(
                f"95th Percentile:   {statistics.quantiles(durations, n=20)[18]:.1f} ms"
            )
...
def main():
    ...
    parser.add_argument(
        "--duration", type=int, default=30, help="Profile duration in seconds"
    )
    ...
    if flamegraph_path:
        ...
        print("2. Look for the hottest paths (widest red bars)")
        print("3. Identify the biggest bottlenecks")
        return 0
    else:
        return 1
