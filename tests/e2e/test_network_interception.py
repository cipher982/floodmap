...
class TestNetworkInterception:
    ...
    async def test_comprehensive_network_analysis(self, page):
        ...
        tile_requests = [r for r in all_requests if any(pattern in r["url"] for pattern in ["/tiles/", "/vector_tiles/", "/flood_tiles/"])]
        other_requests = [r for r in all_requests if r not in html_requests + js_requests + css_requests + tile_requests]
        ...
        if tile_requests:
            ...
            # Group by type
            elevation_tiles = [r for r in tile_requests if "/tiles/" in r["url"] and "/vector_tiles/" not in r["url"] and "/flood_tiles/" not in r["url"]]
            ...
            if null_tile_requests:
                ...
                print(f"✅ No /null/ URLs found in tile requests")
        ...
        assert len(null_requests) == 0, f"Found {len(null_requests)} requests with /null/ URLs"
    ...
    async def test_tile_url_generation_source(self, page):
        ...
        url_logs = [log for log in console_logs if "window.location.origin" in log["text"] or "FETCH_REQUEST" in log["text"] or "NULL_URL" in log["text"]]
