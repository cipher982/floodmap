...
@pytest.mark.asyncio
async def test_zip_slug_page_uses_route_defaults_without_query_params(
    map_page: MapPage,
):
    await map_page.goto_homepage("/zip/33602")
    ...
    robots = await map_page.page.locator("meta[name='robots']").get_attribute("content")
    state = await map_page.get_map_state()
    parsed = urlparse(map_page.page.url)

    assert "ZIP 33602 Flood Map" in title
    assert "Flood map for ZIP 33602 in Tampa, Florida" in heading
    assert "FloodMap USA" in breadcrumb
    assert "Tampa, Florida" in breadcrumb
    assert "ZIP 33602, Tampa, Florida" in breadcrumb
    assert "Broader city flood map" in related_heading
    ...
    assert abs(state["zoom"] - 11.0) < 0.2


@pytest.mark.asyncio
async def test_zip_slug_page_explicit_query_state_overrides_route_defaults(
    map_page: MapPage,
):
    target_url = (
        map_page.page.base_url
        + "/zip/33602?lat=25.76168&lng=-80.19179&zoom=10.30&view=flood&water=6.0"
    )
