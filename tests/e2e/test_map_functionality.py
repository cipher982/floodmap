...
async def test_flood_risk_api(map_page: MapPage):
    ...
    for water_level in test_levels:
        ...
        assert "latitude" in risk_data, f"Missing latitude for water level {water_level}"
        assert "longitude" in risk_data, f"Missing longitude for water level {water_level}"
        assert "elevation_m" in risk_data, f"Missing elevation for water level {water_level}"
...
async def test_performance_metrics(map_page: MapPage):
    ...
    assert "request_processing_seconds" in metrics_text or "Prometheus metrics" in metrics_text