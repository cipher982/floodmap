...
class TestMapLibreAutomated:
    ...
    async def test_maplibre_network_requests_capture(self, page):
        ...
        tile_requests = [r for r in requests_made if "/tiles/" in r["url"] or "/vector_tiles/" in r["url"] or "/flood_tiles/" in r["url"]]
    ...
    async def test_maplibre_html_generation(self, page):
        ...
        if matches:
            ...
            if "/null/" in config_str:
                print(f"❌ FOUND /null/ IN CONFIG!")
                # Find the specific lines with /null/
                lines_with_null = [line.strip() for line in config_str.split('\n') if '/null/' in line]
                for line in lines_with_null:
                    print(f"  - {line}")
            else:
                print(f"✅ No /null/ found in config")
            
            # Check tile URL construction
            if "window.location.origin" in config_str:
                print(f"✅ Using window.location.origin for URLs")
            else:
                print(f"❌ Not using window.location.origin")
        
        else:
            print(f"❌ No MapLibre config found in HTML!")
    ...
    async def test_javascript_execution_debugging(self, page):
        ...
        try:
            ...
            if map_exists:
                ...
                # Get map style
                map_style = await page.evaluate("JSON.stringify(map.getStyle().sources)")
    ...
    async def test_tile_request_debugging(self, page):
        ...
        def handle_response(response: Response):
            if "/tiles/" in response.url or "/vector_tiles/" in response.url or "/flood_tiles/" in response.url:
                if response.ok:
