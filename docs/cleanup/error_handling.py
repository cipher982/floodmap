...
class FloodMapError(Exception):
    ...
    def _get_default_user_message(self, error_code: ErrorCode) -> str:
        ...
        return messages.get(
            error_code, "An error occurred while processing your request."
        )
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            error_handler.logger.error(
                f"Elevation data error in {func.__name__}: {e}", exc_info=True
            )
...
def handle_tile_service_error(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            error_handler.logger.error(
                f"Tile service error in {func.__name__}: {e}", exc_info=True
            )
...
def validate_elevation_data(data: np.ndarray, expected_shape: tuple = None) -> None:
    ...
    if data is None:
        raise ElevationDataError(
            ErrorCode.ELEVATION_DATA_MISSING, "Elevation data is None"
        )
