    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
        ...
        for download in downloads:
            ...
            if available_locally(local_path):
                ...
                futures.append((future, idD))
            downloadMeta[idD].update({'url': url, 'local_path': local_path})
        
        finished = 0
        for future, idD in futures:
            try:
                future.result()
                finished += 1
                logging.info('download_scenes - download finished by {}/{} scenes'.format(finished,len(futures)))
            except DownloadError as e:
                logging.error(f"Failed to download scene {idD}: {str(e)}")
                failed_downloads.append(idD)
    
    logging.info('download_scenes - all download scenes finished')
    if failed_downloads:
        logging.warning(f"Failed to download {len(failed_downloads)} scenes: {', '.join(failed_downloads)}")
