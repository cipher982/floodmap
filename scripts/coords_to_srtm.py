...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Convert coordinates to SRTM tile IDs and verify presence"
    )
    ap.add_argument(
        "--coords",
        type=float,
    ...
    ap.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory containing SRTM GeoTIFFs or compressed tiles",
    )
    args = ap.parse_args()
    ...
    if len(vals) % 2 != 0:
        raise SystemExit(
            "Provide an even number of values for --coords (lat lon pairs)"
        )
