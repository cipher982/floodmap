...
class CoverageExpander:
    ...
    def decompress_tile(self, region: Dict) -> Path:
        ...
        try:
            # Read metadata
            with open(region['json_file'], 'r') as f:
                metadata = json.load(f)
    ...
    def process_regions(self, region_names: List[str], max_workers: int = 4) -> Dict[str, str]:
        ...
        logger.info(f"Processing {len(regions_to_process)} regions with {max_workers} workers")
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            # Track progress
            with tqdm(total=len(regions_to_process), desc="Decompressing tiles") as pbar:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Expand elevation coverage to new regions")
    parser.add_argument('--regions', nargs='+', help='Specific region tiles to process')
    parser.add_argument('--city', choices=get_city_regions().keys(), help='Process predefined city')
    parser.add_argument('--list-available', action='store_true', help='List available regions')
    parser.add_argument('--test', action='store_true', help='Test mode: process Miami only')
    ...
    if args.list_available:
        ...
        for region in regions[:20]:  # Show first 20
            ...
            print(f"  {region['tile_id']}: {bounds['bottom']:.1f}°N, {bounds['left']:.1f}°W ({region['size_mb']:.1f}MB)")
        if len(regions) > 20:
    ...
    try:
        ...
        successful_tifs = [Path(path) for path in results.values() if not path.startswith("ERROR")]
        failed_regions = [region for region, path in results.items() if path.startswith("ERROR")]
