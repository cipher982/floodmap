        ...
        for y in range(y0, y1 + 1):
            ...
            if not has_elevation:
                ...
                if has_roads:
                    ...
                    print(
                        f"PROBLEM TILE: ({x},{y},{z}) - lat {lat_bottom:.3f}-{lat_top:.3f}, lon {lon_left:.3f}-{lon_right:.3f}"
                    )

    print("\n=== SUMMARY ===")
    print(f"Total tiles checked: {total_checked}")
    print(f"Problem tiles found: {len(problem_tiles)}")
    print("These tiles have roads/vectors but no elevation data")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Find tiles with roads but no elevation"
    )
    parser.add_argument(
        "--zoom", "-z", type=int, default=11, help="Zoom level to check"
    )
    parser.add_argument(
        "--wide", action="store_true", help="Check wider area beyond Florida"
    )
