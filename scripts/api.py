...
from downloader import download_scenes, DownloadError
...
class M2M(object):
    ...
    def __init__(self, username=None, password=None, token=None, version="stable"):
        ...
        self.allDatasets = self.sendRequest("dataset-search")
        self.datasetNames = [dataset["datasetAlias"] for dataset in self.allDatasets]
        self.permissions = self.sendRequest("permissions")
        # self.dataset_mapping = {dataset["datasetAlias"]: dataset["datasetName"] for dataset in self.allDatasets if "srtm" in dataset["datasetName"].lower()}
        ...
        if password != None:
            ...
            if token is None:
                ...
                if option in ["p", "P"]:
                    ...
                    config = {"username": username, "token": token}
                    json.dump(
                        config, open(config_file, "w"), indent=4, separators=(",", ": ")
                    )
                    self.loginToken(token)
            else:
                self.loginToken(token)
    ...
    def retrieveScenes(
        self, datasetName, scenes, filterOptions={}, label="m2m-api_download"
    ):
        ...
        def file_exists_and_valid(local_path, expected_size):
        ...
        if requestedDownloadsCount:
            logging.info(
                f"M2M.retrieveScenes - Requested downloads count={requestedDownloadsCount}"
            )
            ...
            for download in downloads:
                idD = str(download["entityId"])
                if idD in downloadMeta:
                    local_path = downloadMeta[idD].get("local_path")
                    expected_size = downloadMeta[idD].get("fileSize", 0)
                    if file_exists_and_valid(local_path, expected_size):
                        existing_files += 1
                        logging.info(f"File already exists and is valid: {local_path}")
                    else:
                        to_download.append(download)
                else:
                    to_download.append(download)

            logging.info(
                f"Found {existing_files} existing valid files. Downloading {len(to_download)} files."
            )

            if to_download:

                def download_with_retry(download, meta):
                    max_retries = 3
                    for attempt in range(max_retries):
                        try:
                            download_scenes([download], meta)
                            return None
                        except DownloadError as e:
                            logging.warning(
                                f"Download failed for {download['downloadId']}. Attempt {attempt + 1}/{max_retries}. Error: {str(e)}"
                            )
                            if attempt == max_retries - 1:
                                logging.error(
                                    f"Failed to download {download['downloadId']} after all retries"
                                )
                ...
                with ThreadPoolExecutor(max_workers=10) as executor:
                    futures = [
                        executor.submit(download_with_retry, download, downloadMeta)
                ...
                if failed_downloads:
                    logging.warning(
                        f"Failed to download {len(failed_downloads)} scenes: {', '.join(failed_downloads)}"
                    )
            else:
...
def retry_connect(
    url, json_data, headers={}, max_retries=5, sleep_seconds=2, timeout=600
):
    retries = 0
    while retries < max_retries:
        try:
            ...
            sec = random.random() * sleep_seconds + 100.0
            time.sleep(sec)
