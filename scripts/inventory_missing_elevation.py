...
def bbox_to_tiles(min_lon: float, min_lat: float, max_lon: float, max_lat: float, z: int) -> List[Tuple[int,int,int]]:
...
def enumerate_srtm_tiles_for_bounds(lat_top: float, lat_bottom: float, lon_left: float, lon_right: float) -> List[Tuple[int,int,str]]:
    eps = 1e-12
...
def find_overlapping_zst_files(lat_top: float, lat_bottom: float, lon_left: float, lon_right: float, data_dir: Path) -> List[Path]:
    eps = 1e-12
    lat_start = math.floor(lat_bottom - eps)
    lat_end = math.ceil(lat_top - eps)
    lon_start = math.floor(lon_left - eps)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Inventory missing SRTM tiles by vector-backed gaps")
    ap.add_argument('--z', type=int, required=True, help='Zoom level to scan (e.g., 10–12 for regional sweeps)')
    ap.add_argument('--bbox', type=float, nargs=4, metavar=('MIN_LON','MIN_LAT','MAX_LON','MAX_LAT'), required=True)
    ap.add_argument('--pretty', action='store_true', help='Pretty-print JSON output')
    ap.add_argument('--data-dir', type=Path, default=ELEVATION_DATA_DIR, help='Directory with compressed elevation tiles (.zst)')
    ap.add_argument('--no-vector', action='store_true', help='Do not query vector tiles; may include ocean false positives')
    args = ap.parse_args()
    ...
    for z, x, y in tiles:
        ...
        overlaps = find_overlapping_zst_files(lat_top, lat_bottom, lon_left, lon_right, args.data_dir)
        if overlaps:
        ...
        srtm_candidates = enumerate_srtm_tiles_for_bounds(lat_top, lat_bottom, lon_left, lon_right)
...
if __name__ == '__main__':
    import asyncio  # local import for event loop
    main()
