...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--input", required=True, help="Directory containing SRTM .tif files")
    g = p.add_mutually_exclusive_group()
    g.add_argument("--region", help="Named region preset (usa, florida, miami)")
    g.add_argument("--bbox", nargs=4, type=float, metavar=("MIN_LON", "MIN_LAT", "MAX_LON", "MAX_LAT"), help="Explicit bbox")
    p.add_argument("--holes", action="store_true", help="Detect interior holes without bbox")
...
def main():
    ...
    if args.json:
        ...
        if "bbox_audit" in reports:
            ...
            b = r['bbox']
            print(f"BBox: lon[{b['min_lon']},{b['max_lon']}], lat[{b['min_lat']},{b['max_lat']}]")
        ...
        if "interior_holes" in reports:
            ...
            env = r.get('envelope', {})
            if env:
                print(f"Envelope lon[{env['lon_min']},{env['lon_max']}], lat[{env['lat_min']},{env['lat_max']}]")
